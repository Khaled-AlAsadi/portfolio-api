...
class CustomTokenObtainPairView(TokenObtainPairView):
    def post(self, request, *args, **kwargs):
        ...
        if not email or not password:
            error_response = {"details": ERROR_MESSAGES["InvalidCredential"]}
            return Response(error_response, status=status.HTTP_400_BAD_REQUEST)

        try:
            user = CustomUser.objects.get(email=email)
            if not user.check_password(password):
                error_response = {"details": ERROR_MESSAGES["WrongPassword"]}
                return Response(error_response, status=status.HTTP_401_UNAUTHORIZED)
        except CustomUser.DoesNotExist:
            error_response = {"details": ERROR_MESSAGES["EmailNotFound"]}
            return Response(error_response, status=status.HTTP_401_UNAUTHORIZED)
        ...
        try:
            ...
            error_response = {"details": ERROR_MESSAGES["InternalError"]}
            return Response(
